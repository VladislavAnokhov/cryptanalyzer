...
import com.javarush.cryptanalyzer.anokhov.constants.FunctionCode;
...
public class Application  {
    ...
    public Result run(){
        Mode parameters = mainController.getView().getParameters();
       ...
       Result result =  function.execute(parameters);
        if (result.getResultOfCode()== ResultCode.GOOD) {
           result =NewFile.newFile(parameters.getFileWayForWrite(),result.getStringBuilder());
